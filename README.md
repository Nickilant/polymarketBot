...
## Сигналы
1. **Крупные ставки (топ-3)** — одиночные крупные входы кошельков (по умолчанию от $1000 за одну ставку), проверка каждые 5 минут.
...
- `/mode insider|probability|hot|both` — выбрать типы сигналов.
...
- `/analysis` — админская сводка последнего анализа.
- `/next_analysis` — админская команда: время следующей рассылки по каждому типу анализа.
