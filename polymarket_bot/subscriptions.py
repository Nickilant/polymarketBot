...
class SubscriptionStore:
    ...
    def ensure_free(self, user_id: int) -> UserSubscription:
        ...
        self._conn.execute(
            """
            INSERT INTO subscriptions (user_id, plan, mode, created_at, last_sent_at, paid_until, is_active, remind_7_for, remind_3_for)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (user_id, FREE_PLAN, "both", _dump_dt(now), None, None, 1, None, None),
        )
        self._conn.commit()
        return self.get(user_id)  # type: ignore[return-value]
    ...
    def active_users_due(self, now: datetime) -> list[UserSubscription]:
        rows = self._conn.execute("SELECT * FROM subscriptions WHERE is_active = 1").fetchall()
        due: list[UserSubscription] = []
        for row in rows:
            sub = self._row_to_sub(row)
            if sub.last_sent_at is None or now - sub.last_sent_at >= sub.interval:
                due.append(sub)
        return due
    ...
    def mark_sent(self, user_id: int, sent_at: datetime) -> None:
        self._conn.execute(
            "UPDATE subscriptions SET last_sent_at = ? WHERE user_id = ?",
            (_dump_dt(sent_at), user_id),
        )
        self._conn.commit()
